...
Croquis.Brush = function () {
    ...
    this.down = function(x, y, pressure, azimuthAngle) {
        ...
        let pressure_opacity = (1 + Math.min(dynamicPressureOpacity, 0)) - pressure * dynamicPressureOpacity;
        if (scale > 0) {
            ...
            else
            {
                if (USE_BLEND_MODE)
                {
                    oContext.globalAlpha = flow * (pressure == 0 ? spPrevOpacity : pressure_opacity);
                }
                else
                {
    ...
    this.move = function(x, y, pressure, azimuthAngle) {
        ...
        {
            scale = (1 + Math.min(dynamicPressureSize, 0)) - pressure * dynamicPressureSize;
            pressure_opacity = (1 + Math.min(dynamicPressureOpacity, 0)) - pressure * dynamicPressureOpacity;
        }
        if (context == null)
            throw 'brush needs the context';
        if (scale <= 0) {
        ...
        if (ld < drawSpacing) {
            lastX = x;
            lastY = y;
            drawTo(lastX, lastY, (pressure == 0 ? spCurScale : scale, pressure), azimuthAngle);
            delta -= drawSpacing;
        } else {
            while(delta >= drawSpacing) {
                ...
                if (USE_BLEND_MODE)
                    oContext.globalAlpha = opacity * (pressure == 0 ? spPrevOpacity : pressure_opacity);
                else
                    context.globalAlpha = opacity * (pressure == 0 ? spPrevOpacity : pressure_opacity);
                if (flow > 0)
                {
                    drawTo(lastX, lastY, (pressure == 0 ? spPrevScale : prevScale), pressure, azimuthAngle);
